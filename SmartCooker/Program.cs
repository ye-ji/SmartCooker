...
namespace SmartCooker
{
    ...
    /* http://cafe.daum.net/nowcampus/a1JS/9
     * 쌀넣기 ->물넣기->쌀씻기->배수
     *       ->물넣기->쌀씻기->배수
     *                           ->취사 ->보온
     * 
    ...
    class Program
    {
        ...
        struct RiceCookerInfo
        {
            ...
            public int Rice;  // 쌀의 양. g 기준. 출력 할 때 환산
            public int Water; // 물의 양. mL 기준. 출력 때 환산
        ...
        // ststic main이 호출하므로 모든 메소드는 static으로 만들겠습니다. 이번은!
        // class를 신규로(인스턴스로) 생성하지 않을 것이기 때문에!!!
        // Menu 출력 메소드
            ...
            while(true)
            {
                for(int i = 0; i < MenuItem.Length; i++)
                {
                    if(MainMenuIndex == i)
                    {
                        ...
                        // 아래를 쓰지 않으면, 모든 내용에 대해 위 옐로우/블랙이 계속 적용되므로 아래 재설정이 반드시 필요!
